...
export function defaultBoardColumns(listIds: string[]): string[][] {
  return [['today', 'tomorrow', 'week'], ...listIds.map((id) => [id])]
}
...
export function insertCardOnBoard(
  ...
  if ('newColumnAt' in target) {
    ...
    // Removing an only-card column shifts later column indexes left.
...
export function ensureBoardHasCards(
  ...
  const present = new Set(flattenBoard(columns))
  const next = columns.map((c) => [...c]).filter((c) => c.length > 0)

  for (const id of PLAYLIST_CARD_IDS) {
    if (!present.has(id)) {
      if (next[0]) next[0].push(id)
      else next.push([id])
      present.add(id)
    }
  }
