...
- **Playlist** — Today / Tomorrow / This Week, one day at a time. The day is changed from the strip at the top: tap a day, or flick along the strip. A Now card sits on top and holds one task, which is not repeated in the queue below it: when a task is timed and close it takes the card, otherwise it suggests a single untimed task you can re-roll; either way the card hugs its title and the one round Done beside it. The day's commitments and tasks share one run of lines, ordered `By time` or `Custom` from a small `Sort` at the top right of the queue — and in `Custom`, holding a row lets you drag it where you want it. Completed work folds away behind an inline `Completed · Clear`, and completing, clearing or deleting raises a brief Undo toast.
- **Lists** — every context list, pinned ones in their own section. A row is a dot, a name, and a bare number when something is waiting in it; a list with nothing waiting says nothing, and no row carries a control. Tap a list to open it where it stands as a light run of bullets; the full page is one step further in. Pin with a flick right — the row moves with your thumb and uncovers the pin it will set — or from the list's own menu, and hold a row to drag it into a new order. When capture cannot place something, a triage banner offers the raw text, a few lists, and "Also add to Today".
...
Rows are the same everywhere and nothing is singled out by colour: checkbox, time on the left, title, then the owning list named plainly — and only when it differs from the surface you are on. Nothing wears a card except the Now card; lists and queues are lines held apart by one hairline, drawn the same way and at full width wherever rows meet, so the page reads as one ruled column rather than a stack of tiles. A time that is set wears a small chip in the row's own colour, so "this happens at a time" lands before you have read the title; an empty slot is a quiet dash, and tapping it sets a time without leaving the row.

On the Playlist and inside a list, a row is also the control. Pull it right to complete it and it leaves for the Completed fold, with Undo on the toast. Pull it left and it holds open on **Move** and **Delete** — Move offers the three days and every list, and applies on the tap. The row follows your thumb the whole way and the label underneath lights up once letting go would commit, so nothing fires by surprise.
...
Tapping a task opens one lean sheet: the time beside the title, the classifier's top few lists as chips (with a searchable `More…` behind them), and a Today / Tomorrow / This week segment that clears on a second tap. Delete sits bottom-left and Save bottom-right; the X walks away from the edit. Nothing in it grabs the keyboard on open.
